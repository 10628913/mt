    ...
    */
    ...
    public function menuAdd(){
        if(IS_POST){
            ...
            $where['parent_id'] = 0;
            $data = M('Admin_menu')->where($where)->select();
    ...
    public function menuEdit(){
        if(IS_POST){
            $id = $where['id'] = I("id");
            $data = $_POST['info'];
            M('Admin_menu')->where($where)->save($data);
            adminLog("修改后台菜单".$data["name"]);
            $this->success("操作成功");
        }else{
            $id = $where['id'] = I("id");
            $whereParent['parent_id'] = 0;
            $parentData = M('Admin_menu')->where($whereParent)->select();
            $this->assign("parentData",$parentData);
            $data = M('Admin_menu')->where($where)->find();
            $this->assign($data);
            layout(false);
    ...
    public function menuDelete(){
        if(IS_POST){
            ...
            $menuData = M('Admin_menu')->where($whereData)->find();
            if($menuData){
                $this->error("请先删除子菜单");
            }else{
                $where['id'] = $id;
