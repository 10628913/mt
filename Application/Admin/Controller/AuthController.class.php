...
class AuthController extends BaseController {
    ...
    */
    public function index(){
        ...
        $data = M('Auth_rule')->order('sort desc,id desc')->where(array('is_delete'=>0))->select();
    ...
    */
    public function ruleEdit(){
        if(IS_POST){
            ...
            $parentData = M("Auth_rule")->where($whereParent)->select();
            $this->assign("parentData",$parentData);
            $data = M("Auth_rule")->where($where)->find();
            $this->assign($data);
            layout(false);
    ...
    */
    public function ruleDelete(){
        if(IS_POST){
            ...
            $ruleData = M("Auth_rule")->where($whereData)->find();
            if($ruleData){
                $this->error("请先删除子规则");
            }else{
                $where['id'] = $id;
